...
//echo $query."<br>";
	...
	if ($state==ST_REGISTERING)	  { $move_item_sel = ST_REG_DONE; }
...
?>
	...
	<tr>
		<td colspan="2" class="text-center">
			...
			<? if ($state==ST_REGISTERING || $state==ST_FIX_DONE) { ?>
...
<? if ($state==ST_REGISTERING || $state==ST_REG_DONE || $state==ST_FIX_DONE) {?> <!-- 접수중/접수완료/수리완료 페이지에만 표시-->
	...
	<tr>
		<th>
		<? if ($state==ST_REGISTERING || $state==ST_FIX_DONE) { echo "택배 접수완료 처리"; } else if ($state==1) { echo "송장번호(회수용) 입력 처리"; } ?>
		</th>
		<td>
