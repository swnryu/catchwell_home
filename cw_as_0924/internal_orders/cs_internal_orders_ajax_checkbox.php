...
if($name=="status")
{
	...
	{
		if($dbname=="cs_internal_orders")
		{
			//완료처리
			$db->update($dbname, "status=1 where idx='$idx[$i]' and status=0 ");

			//LOG
			$db->insert("admin_log","userid='$_SESSION[ADMIN_USERID]', contents='cs_internal_orders_edit', ip='$_SERVER[REMOTE_ADDR]', udate=now(), comment='$idx[$i] $ADMIN_NAME'");
		} 
	}
}
