...
$db_name	= "cs_internal_orders";

$return_url = isset($_POST['return_url']) ? $_POST['return_url'] : "cs_internal_orders_edit.php";
//echo $return_url;
...
else if($mode=="edit")
{
	...
	{
		//관리자 로그
		$db->insert("admin_log","userid='$_SESSION[ADMIN_USERID]', contents='cs_internal_orders_edit', ip='$_SERVER[REMOTE_ADDR]', udate=now(), comment='$idx $ADMIN_NAME'");
...
else if ($mode=="del") 
{
	...
	{
		//관리자 로그
		$db->insert("admin_log","userid='$_SESSION[ADMIN_USERID]', contents='cs_internal_orders_del', ip='$_SERVER[REMOTE_ADDR]', udate=now(), comment='$idx $ADMIN_NAME'");
