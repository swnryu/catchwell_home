...
<h4 class="page-header"><?if($menu==S_PARTS_SHIPMENT) {echo "사내판매 출고 완료 조회";} else {echo "사내판매 출고 리스트";}?> </h4>
...
<?if ($menu==S_PARTS_SHIPMENT) { ?>        
        ...
        <a href="cs_internal_orders_excel_download.php?<?echo ($param_url);?>" class="btn btn-success btn-sm">엑셀 다운로드 ALL</a>
<? } else { ?>
...
<? if ($menu==S_PARTS_LIST) {?>
<form method="post" name="upload_form" class="form-inline" enctype="multipart/form-data" action="<?='cs_internal_orders_excel_upload.php'?>" >
	<table class="table table-bordered">
	<colgroup>
	<col width="15%">
	<col width="*">
...
<? if ($menu==S_PARTS_LIST) {?>
        <td class="text-center"><a href="./cs_internal_orders_edit.php?idx=<? echo $row['idx']."&return_url=".urlencode($return_url); ?>" class="btn btn-primary btn-sm">보기</a></td>
<? } else { ?>
        <td class="text-center"><a href="./cs_internal_orders_edit.php?shipment=1&idx=<? echo $row['idx']."&return_url=".urlencode($return_url);?>" class="btn btn-primary btn-sm">보기</a></td>
<? } ?>
        </tr>
    ...
    if(name=="status") {
        ...
        if(  $("input:checkbox[name='check_list']").is(":checked") ){
            ...
            $.ajax({
                ...
                url : "cs_internal_orders_ajax_checkbox.php",
                type: "post",
                data: postData,
        ...
        if(  $("input:checkbox[name='check_list']").is(":checked") ){
            ...
            $.ajax({
                ...
                success:function(obj){ 
                    ...
                    var filename = "../temp"+"/사내판매출고_"+y+m+d+".xlsx";
