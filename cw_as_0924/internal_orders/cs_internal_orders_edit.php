...
$table = "cs_internal_orders";

$row = $db->object($table,"where idx='$idx'");

//사내판매제품리스트를 데이터베이스에서 읽어옴
$products = [];

$result = $db->select("products_internal_sales", "where 1", "product_name, product_price");

while ($row1 = $result->fetch_object()) {
    $products[$row1->product_name] = (int)$row1->product_price;
}
...
?>
	...
	<h4 class="page-header"><?if($idx>0) {echo "사내판매 출고 상세 정보"."($idx)";} else {echo "사내판매 출고 등록";} ?>  </h4>

	<form method="post" action="cs_internal_orders_edit_ok.php" name="tx_editor_form" enctype="multipart/form-data" >
		...
		<td>
			<div class="form-inline" >
				<select id="product_name"
						...
						onchange="changeProductCategory(this)">
				<?php foreach($products as $name => $price){ ?>
					<option value="<?php echo $name; ?>"
							data-price="<?php echo (int)$price; ?>"
							<?php if($product_name == $name) echo "selected"; ?>>
						<?php echo $name; ?>
					</option>
				<?php } ?>
				</select>
	...
	<tr>
		...
		<input type="text"
		   ...
		   placeholder="₩"
		   autocomplete="off"
		   value="<?=number_format($row->parts_price)?>"
		   oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
		   onkeyup="this.value=number_format(this.value)">(원)
		</div>
		</td>
	</tr>
	<tr>
		<th>판매경로</th>
		<td>
			<textarea class="form-control input-sm" rows="2" name="reason" placeholder="판매 경로 입력" autocomplete="off" ><?=$row->reason?></textarea>
		</td>
	</tr>
<? if($mode != "new") { ?>
	<tr>
...
<script>
$(function () {
    // 최초 로딩 시에도 선택된 모델의 가격을 자동 반영
    changeProductCategory(document.getElementById('product_name'));
});
</script>
...
function deleteit() {
	...
	if (confirm("삭제할까요?")) 
	{
		if (confirm("삭제된 데이터는 복구할 수 없습니다. 삭제할까요?")) 
		{
			...
			form.mode.value = 'del';
			form.action = "cs_internal_orders_edit_ok.php";
			form.submit();
		}
	}
}
...
function changeProductCategory(obj)
{
    ...
    // 가격 input에 콤마 포함해서 반영
    $('#parts_price').val(number_format(price));
