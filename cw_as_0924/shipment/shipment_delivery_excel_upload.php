...
//NO/B-운송장번호/C-접수일자/D-계약품목/E-송하인/F-받는분/G-전화번호/H-휴대번호/I-주소/J-인수자/집화사업담당/집화지점/집화점소/집화계획SM/집화일자/배달사업담당/배달지점/배달점소/배송계획SM/배달일자/미배달스캔일/미배달사유/W-주문번호/품명/단품코드
	...
	try {
		...
		//20220105
		$col_name = "F";
		if ($objWorksheet->getCell('F1')->getValue()=='받는분') {
			$col_name = "F";
		} else if ($objWorksheet->getCell('G1')->getValue()=='받는분') {
			$col_name = "G";
		}

		$col_oid = "W";
		if ($objWorksheet->getCell('W1')->getValue()=='주문번호') {
			$col_oid = "W";
		} else if ($objWorksheet->getCell('X1')->getValue()=='주문번호') {
			$col_oid = "X";
		}

		$col_product = "X";
		if ($objWorksheet->getCell('X1')->getValue()=='품명') {
			$col_product = "X";
		} else if ($objWorksheet->getCell('Y1')->getValue()=='품명') {
			$col_product = "Y";
		}
		...
		for ($i = $start_Row ; $i <= $maxRow ; $i++) 
		{
			...
			if (true) {	
				...
				$date = $objWorksheet->getCell('C' . $i)->getValue();//택배접수일자
				$date = date("Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($date)); 
				$tracking = $objWorksheet->getCell('B' . $i)->getValue();//운송장번호
