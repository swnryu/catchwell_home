...
if($name=="export2excel") {
	...
	for($i=0;$i<count($idx);$i++) {	
            ...
            if ($row->content == '가죽케이스 : 다크블루')
            {
                $gift = $row->japum . '_가죽케이스_다크블루';
                $gift_opt = '다크블루';
            }
            else if ($row->content == '가죽케이스 : 파스텔블루')
            {
                $gift = $row->japum . '_가죽케이스_파스텔블루';
                $gift_opt = '파스텔블루';
            }
            else if ($row->content == '가죽케이스 : 에쉬베이지')
            {
                $gift = $row->japum . '_가죽케이스_에쉬베이지';
                $gift_opt = '에쉬베이지';
            }
            else if ($row->content == '가죽케이스 : 베이비핑크')
            {
                $gift = $row->japum . '_가죽케이스_베이비핑크';
                $gift_opt = '베이비핑크';
            }
            else if ($row->content == '가죽케이스 : 네온오렌지')
            {
                $gift = $row->japum . '_가죽케이스_네온오렌지';
                $gift_opt = '네온오렌지';
            }
			else if ($row->content == '가죽케이스 : 컬러 랜덤')
            {
                $gift = $row->japum . '_가죽케이스_컬러랜덤';
                $gift_opt = '컬러랜덤';
            }
            
            else if ($row->content == '매직스틱:크림화이트')
            {	
                $gift = '매직스틱_크림화이트';
                $gift_opt = '크림화이트';
            }
            else if ($row->content == '매직스틱:블로썸피치')
            {	
                $gift = '매직스틱_블로썸피치';
                $gift_opt = '블로썸피치';
            }
            else if ($row->content == '매직스틱:라벤더핑크')
            {	
                $gift = '매직스틱_라벤더핑크';
                $gift_opt = '라벤더핑크';
            }
            else if ($row->content == '매직스틱:실버그레이')
            {	
                $gift = '매직스틱_실버그레이';
                $gift_opt = '실버그레이';
            }

            else if ($row->content == '스마트체중계')
            {	
                $gift = 'CW44';
                $gift_opt = 'CW44';
            }
            else if ($row->content == '보풀제거기')
            {	
                $gift = 'R6';
                $gift_opt = 'R6';
            }
            ...
            ->setCellValue(sprintf("B%s", $colIdx), $gift.'_사은품') 
            ->setCellValue(sprintf("E%s", $colIdx), 1)
            ->setCellValue(sprintf("F%s", $colIdx), '스나이퍼2 포토이벤트'/*$row->event_name*/)
            ->setCellValue(sprintf("G%s", $colIdx), '' /*$row->oid*/)
            ->setCellValue(sprintf("H%s", $colIdx), '이벤트사은품' /*$row->shoppingmall*/)
            ->setCellValue(sprintf("I%s", $colIdx), $row->name)
            ->setCellValue(sprintf("K%s", $colIdx), $row->hp)
            ->setCellValue(sprintf("L%s", $colIdx), $row->hp)
            ->setCellValue(sprintf("M%s", $colIdx), $row->add1." ".$row->add2)
        ...
        $db->insert("admin_log","userid='$_SESSION[ADMIN_USERID]', contents='excel_dl_snp', ip='$_SERVER[REMOTE_ADDR]', udate=now(), comment='$fileName' ");
