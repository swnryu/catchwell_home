<?
//20231005 송장번호 업로드 새로 생성.

include("../def_inc.php");
include("event_def.php");
$mod	= M_EVENT;
$menu	= S_EVENT_GIFT; //20230508 생성
include("../header.php");

$db_name	    = "cs_online_event";
//$return_url	= "online_event.php";
$return_url	    = "online_event_gift.php";
...
if( $_FILES['userfile']['size'] > 0 ) 
{
            ...
            else
            {
                ...
                if (true) 
                {
                  ...
                  //A 발주날짜, B 모델,     C , D 요청자, E 수량, F idx, G 주문번호, H 구매처, I 수령자명, J 재출고송장번호, K 일반전화, L 핸드폰, M 주소, N 배송메세지, O 사방넷주문번호 
                    A 발주날짜, B 사은품명, C , D 핀번호, E 수량, F idx, G 주문번호, H 구매처, I 수령자명, J 송장번호, K 일반전화, L 핸드폰, M 주소, N 배송메세지, O 사방넷주문번호 
                    */
                    $ex_order_date                              = $objWorksheet->getCell('A' . $i)->getValue(); //발주날짜 
                    //$ex_model_name                              = $objWorksheet->getCell('B' . $i)->getValue(); //모델
                    $ex_gift                                    = $objWorksheet->getCell('B' . $i)->getValue(); //사은품
                    $ex_pin_num                                 = $objWorksheet->getCell('D' . $i)->getValue(); //요청자대신 pin_num
                    $ex_amount                                  = $objWorksheet->getCell('E' . $i)->getValue(); //수량
                    $ex_idx                                     = $objWorksheet->getCell('F' . $i)->getValue(); //idx (접수번호)
                    $ex_oid                                     = $objWorksheet->getCell('G' . $i)->getValue(); //주문번호

                    $ex_shoppingmall                            = $objWorksheet->getCell('H' . $i)->getValue(); //구매처
                    $ex_name                                    = $objWorksheet->getCell('I' . $i)->getValue(); //수령자명
                    $ex_tracking                                = $objWorksheet->getCell('J' . $i)->getValue(); //송장번호

                    $ex_tel                                     = $objWorksheet->getCell('K' . $i)->getValue(); //일반전화                    
                    $ex_hp                                      = $objWorksheet->getCell('L' . $i)->getValue(); //모바일전화
                    $ex_add                                     = $objWorksheet->getCell('M' . $i)->getValue(); //주소 주의 
                    ...
                    echo "excel ex_gift: "                      . $ex_gift . "<br>";
                    echo "excel ex_pin_num: "                   . $ex_pin_num . "<br>";
                    echo "excel ex_amount: "                    . $ex_amount . "<br>";
                    echo "excel ex_idx: "                       . $ex_idx . "<br>";    
                    echo "excel ex_oid: "                       . $ex_oid . "<br>";
                    echo "excel ex_shoppingmall: "              . $ex_shoppingmall . "<br>";
                    echo "excel ex_name: "                      . $ex_name . "<br>";
                    echo "excel ex_tracking: "                  . $ex_tracking . "<br>";   
                    echo "excel ex_tel: "                       . $ex_tel . "<br>";
                    echo "excel ex_hp: "                        . $ex_hp . "<br>";
                    echo "excel ex_add: "                       . $ex_add . "<br>";
                    ...
                    */
                    ...
                    if ($ex_idx == '' || $ex_hp == '') 
                    {
                        //echo "<br>"."ex_idx : ". $ex_idx . "  ex_hp : ". $ex_hp . "<br>";//주석
                        continue;
                    }
                    ...
                    //gdate 구매일, udate 접수일 
                    //20231004 접수일에서 3달까지 조회되게 수정
                    $sql = "SELECT * FROM $db_name where udate BETWEEN DATE_ADD(NOW(),INTERVAL -3 MONTH ) AND NOW() ";
                    ...
                    for ($j=0; $j<$row_cnt+1; $j++)//db 행갯수 만큼 돈다.
                    {
                        ...
                        //20231004
                        //echo "idx : ". $idx. " name : " . $name. " udate : ".$udate. " hp : ".$hp. " tracking_num : " .$tracking_num. " status : " .$status. "<br>";//주석

                        //if ($ex_idx == $idx || $hp == $hp)//엑셀에있는 idx와 db에 있는 idx가 같을 경우에만 실행
                        if ($ex_idx == $idx)//엑셀에있는 idx와 db에 있는 idx가 같을 경우에만 실행
                        {
                             //echo " ex_idx : " . $ex_idx. ", idx : ". $idx. ", ex_hp : " . $ex_hp. ", hp : ". $hp. "<br>";//주석
                             $cnt_suc++;

                             if( ($tracking_num == '') &&  ($status == 99) )  //송장번호가 없고 상태값이 완료이면
                             {
                               //echo " 0 tracking_num : " . $tracking_num. ", status : " . $status. "<br>";//주석                                                                                                              
                                                                            //업데이트시 엑셀에서 읽어온 값을 넣는다.
                                $query = "update $db_name set tracking_num='$ex_tracking' where idx ='$idx'  ";

                                if (mysqli_query($db->db_conn, $query))
                                {
                                    //echo "0 update 성공"."<br>";//주석
                                }
                                else
                                {
                             ...
                             else if( ($tracking_num != '') && ($status == 99) )  //송장번호가 있고 상태값이 완료이면 (송장이 있어도 덮어쓰기 가능하게 추가)
                             {
                                //echo " 1 tracking_num : " . $tracking_num. ", status : " . $status. "<br>";//주석                                                                                                       
                                                                            //업데이트시 엑셀에서 읽어온 값을 넣는다.
                                 $query = "update $db_name set tracking_num='$ex_tracking' where idx ='$idx'  ";
                               
                               if (mysqli_query($db->db_conn, $query))
                               {
                                   //echo "1 update 성공"."<br>";//주석
                               }
                               else
                               {
                                   $tools->errMsg('데이터베이스 업데이트 오류가 발생하였습니다. 관리자에게 문의하세요.(1)'); 
                             ...
                             {
                                //echo " 2 tracking_num : " . $tracking_num. ", status : " . $status. "<br>";//주석
                             }
                        }
                        else
                        {
	...
	//@unlink($_FILES['userfile']['tmp_name']);//20231011 
	//@unlink($file_pathname);//20231011
