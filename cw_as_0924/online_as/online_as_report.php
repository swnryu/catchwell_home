...
include("../def_inc.php");
$mod	= M_AS;
$menu	= S_AS_REPORT;//20230707 AS 전체 검색 
...
//	$date_from = isset($_POST["date_from"]) ? $_POST["date_from"] : date("Y-m-d", strtotime($day." -1 week"));
	...
	//$query	= "select * from $table where update_time between date('$date_from') and date('$date_to2') and (process_state > $state_fixdone) ";
	$query	= "select * from $table where update_time between date('$date_from') and date('$date_to2')  ";//20230707 출고외에도 나오게 수정.
...
*/
	...
	//$query = "select * from $table where update_time between date('$date_from') and (date('$date_to2')) and (process_state > $state_fixdone) ";
	$query = "select * from $table where update_time between date('$date_from') and (date('$date_to2')) ";//20230707 출고외에도 나오게 수정. 여기가 중요
...
//echo "row2->cnt : ".$row2->cnt;//주석
	...
	<tr>
		...
		<td class="text-center"><?if ($row[process_state] == 4){?><a href="<?=$site_url?>/online_as/online_as_shipment.php"> <?}else {?><a href="<?=$site_url?>/online_as/online_as.php?state=<?echo $row[process_state];?>" target="_blank"><?}?>
		<?echo $proc_state[$row[process_state]]?></td>
		...
		<!--20230707 상세관리에 수정버튼 생성 및 상태값에 따라 링크 페이지로 갈 수 있게 수정 -->
		<td class="text-center">
			<?if ($row[process_state] == 0)//접수중
			{?>
				<a href="./online_as_edit.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_REGISTERING ?>" class="btn btn-default btn-sm">
			<?}
			else if ($row[process_state] == 1)//접수완료
			{?>
				<a href="./online_as_edit.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_REGDONE ?>" class="btn btn-default btn-sm">
			<?}
			else if ($row[process_state] == 3)//수리완료
			{?>
				<a href="./online_as_edit.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_FIXDONE ?>" class="btn btn-default btn-sm">
			<?}
			else if ($row[process_state] == 4)//출고
			{?>
				<a href="<?=$site_url?>/online_as/online_as_edit.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_SHIPMENT ?>" class="btn btn-default btn-sm"> 
			<?}
			else 
			{?>
				<a href="./online_as_edit.php?idx=<? echo $row[idx] ?>&from=<? echo $menu ?>" class="btn btn-default btn-sm">
			<?}?>
		수정</a>
	    </td>
		...
		<!--20230707 상세관리에 보기버튼 수정 및 상태값에 따라 링크 페이지로 갈 수 있게 수정 -->
		<td class="text-center">
		<?if ($row[process_state] == 0)//접수중
			{?>			
				<a href="./online_as_view.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_REGISTERING ?>" class="btn btn-primary btn-sm">
			<?}
			else if ($row[process_state] == 1)//접수완료
			{?>
				<a href="./online_as_view.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_REGDONE ?>" class="btn btn-primary btn-sm">
			<?}
			else if ($row[process_state] == 3)//수리완료
			{?>
				<a href="./online_as_view.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_FIXDONE ?>" class="btn btn-primary btn-sm">
			<?}
			else if ($row[process_state] == 4)//출고
			{?>
				<a href="./online_as_view.php?idx=<? echo $row[idx] ?>&from=<? echo $menu = S_AS_SHIPMENT ?>" class="btn btn-primary btn-sm">
			<?}
			else 
			{?>
				<a href="./online_as_view.php?idx=<? echo $row[idx] ?>&from=<? echo $menu ?>" class="btn btn-primary btn-sm">
			<?}?>
			보기
			</a>
		</!td>
