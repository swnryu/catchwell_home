...
if($name=="export2excel") {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                //->setCellValue('F1', '')//20230809 요청 널값이어서 제거
...
//db : cancellation_order
            ...
            ->setCellValue(sprintf("D%s", $colIdx), $row->admin_name /**/)//20230809 요청자(담당자추가)
            ...
            //->setCellValue(sprintf("G%s", $colIdx), $row->order_id)//주문번호
           // ->setCellValueExplicit(sprintf("E%s", $rowIdx), $row['order_id'], PHPExcel_Cell_DataType::TYPE_STRING)  
            ...
            //->setCellValue(sprintf("H%s", $colIdx), $row->company_name)//업체명
            //20230913 업체명에서 구매처로 변경 요청 건 수정
            ->setCellValue(sprintf("H%s", $colIdx), $row->shopping_mall)//구매처

            ->setCellValue(sprintf("I%s", $colIdx), $row->customer_name)//수령자명

            //->setCellValue(sprintf("J%s", $colIdx), $row->tracking)//송장번호
            ...
            ->setCellValue(sprintf("L%s", $colIdx), $row->phone)//핸드폰
