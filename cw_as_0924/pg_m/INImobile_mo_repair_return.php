...
<style>
    :root { --primary-color: #007AFF; --success-color: #34C759; --bg-gray: #F8F9FA; --text-dark: #212529; --text-muted: #6C757D; --card-radius: 16px; }
    ...
    .main-card { background: #fff; border-radius: var(--card-radius); box-shadow: 0 4px 20px rgba(0,0,0,0.05); padding: 24px; margin-bottom: 24px; }
    .page-title { font-size: 20px; font-weight: 700; margin-bottom: 8px; text-align: center; }
    .page-subtitle { font-size: 14px; color: var(--text-muted); text-align: center; margin-bottom: 24px; }
    .complete-badge { display: block; width: fit-content; margin: 0 auto 12px; background-color: #E8F9EE; color: var(--success-color); font-size: 13px; font-weight: 700; padding: 6px 14px; border-radius: 50px; }
    ...
    .vbank-account-box { background: #f8fbff; border: 1px solid #e1eeff; border-radius: 10px; padding: 15px; text-align: center; margin-top: 10px; cursor: pointer; transition: all 0.2s; }
    .vbank-account-box:active { background: #edf5ff; transform: scale(0.98); }
    .vbank-account-number { display: block; font-size: 20px; font-weight: 800; color: var(--primary-color); margin-bottom: 4px; letter-spacing: -0.5px; }
    .copy-hint { font-size: 11px; color: var(--text-muted); }
    .info-box { background-color: #fff9f0; border-radius: 12px; padding: 16px; margin-bottom: 24px; }
    .info-box h4 { font-size: 14px; font-weight: 700; color: #d48806; margin-bottom: 8px; }
    .info-box ul { padding-left: 18px; margin-bottom: 0; font-size: 13px; color: #855d10; }
    #toast-msg { visibility: hidden; min-width: 200px; background-color: #333; color: #fff; text-align: center; border-radius: 8px; padding: 12px; position: fixed; z-index: 1000; left: 50%; bottom: 30px; transform: translateX(-50%); font-size: 14px; }
...
<div class="container-mobile">
    ...
    <main class="main-card">
        ...
        <h2 class="page-title">수리비 입금 안내</h2>
        <p class="page-subtitle">아래 계좌로 수리비를 입금해 주세요.</p>

        <?php if ($P_STATUS === "00" && !empty($out)): ?>
            <div class="vbank-info-table">
                <div class="vbank-row">
                    <span class="vbank-label">접수번호</span>
                    <span class="vbank-value"><?php echo htmlspecialchars(str_replace('_R', '', $out["P_OID"])); ?></span>
                </div>
                <div class="vbank-row">
                ...
                <div class="vbank-row">
                    <span class="vbank-label">수리비 금액</span>
            ...
            <div class="info-box">
                ...
                <ul>
                    ...
                    <li>입금이 확인되면 수리 작업이 자동으로 진행됩니다.</li>
                </ul>
            </div>
        <?php else: ?>
